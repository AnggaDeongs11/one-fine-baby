...
class WCVendors_Pro_Commission_Controller {
	...
	public function process_commission( $commission, $product_id, $product_price, $order, $qty ) {
		...
		if ( count( $categories ) == 1 && 'single_select' === get_option( 'wcvendors_category_display' ) ) {
		...
		if ( $product_commission_type ) {
			...
			$global_commission_type = get_option( 'wcvendors_commission_type' );
		...
		if ( ! empty( $coupons ) ) {
			...
			foreach ( $coupons as $coupon ) {
				...
				if ( ! $coupon_obj->is_valid_for_product( $product ) ) continue;
				...
				if ( $coupon_owner == $product_owner || in_array( 'administrator', $coupon_user->roles ) ) {
					$discount_amount += $coupon_obj->get_discount_amount( $product->get_price() );
		...
		return apply_filters( 'wcv_process_commission', $commission, $product_id, $product_price, $order, $qty );
	...
	public function get_tier( $value, $from = 'global', $key = 'vendor_sales', $id = 0 ) {
		...
		foreach ( $tiers as $tier ) {
			if ( $value < $tier['value'] && 'less_than' === $tier['rule'] ) {
				...
				return $tier;
			} elseif ( $value > $tier['value'] && 'more_than' === $tier['value'] ) {
				return $tier;
			}
		}
		return array();
	...
	public function calculate_order_commissions( $order ) {
		...
		foreach ( $order->get_items() as $item ) {
			...
			$commission = is_numeric( $commission ) ? $this->process_commission( $commission, $product_id, $product_price, $order, $qty ) : 0;
