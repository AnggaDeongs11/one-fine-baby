...
class WCVendors_Pro_Product_Form {
		...
		WCVendors_Pro_Form_helper::submit(
			apply_filters(
				...
				array(
					'id'    => 'draft_button',
					'value' => $button_text,
					'class' => 'btn btn--secondary btn-long mr-2',
				)
			)
		);
	...
	public static function categories( $post_id, $multiple = false ) {
		...
		if ( 'yes' != get_option( 'wcvendors_hide_product_basic_categories' ) ) {
			...
			} elseif ( get_option( 'wcvendors_category_display' ) == 'single_select' ) {
	...
	public static function categories_dropdown( $post_id, $multiple = false ) {
		...
		if ( 'yes' != get_option( 'wcvendors_hide_product_basic_categories' ) ) {
			...
			$custom_attributes = 'yes' === get_option( 'wcvendors_required_product_basic_categories' ) ? array(
				'required'                   => '',
	...
	public static function tags( $post_id, $multiple = false ) {
		...
		$required_field = 'yes' === get_option( 'wcvendors_required_product_basic_tags' ) ? array(
			'required'                   => '',
...
//		$type_box  = '<div class="wcv-cols-group wcv-horizontal-gutters"><div class="all-50 small-100">';
		$type_box = '<div class="col-12 col-md-6 d-none">';
		$type_box .= '<label>' . __( 'Product Type', 'wcvendors-pro' ) . '</label>';
//		$type_box .= '<div class="control select">';
		$type_box .= '<select id="product-type" name="product-type" class="form-control">';
...
//		$type_box .= '</div>'; // grid
		...
		$type_box .= '<div class="col-12 col-md-6 justify-content-end d-none">';
//		$type_box .= '<div class="control-group"> <br />';

		// Only output the list if there is options
		if ( ! empty( $product_type_options ) ) {
			$type_box .= '<ul class="control unstyled inline" style="padding: 0; margin:34px 0 0;">';
			...
			foreach ( $product_type_options as $key => $option ) {
				...
				$type_box .= '<li class="' . esc_attr( $option['wrapper_class'] ) . ' "><input style="margin-right: 5px;" type="checkbox" name="' . esc_attr( $option['id'] ) . '" id="' . esc_attr( $option['id'] ) . '" ' . checked( $selected_value, true, false ) . ' /><label for="' . esc_attr( $option['id'] ) . '" class="' . esc_attr( $option['wrapper_class'] ) . ' " data-tip="' . esc_attr( $option['description'] ) . '">' . esc_html( $option['label'] ) . '</label></li>';
			}
	...
	public static function sale_price( $post_id ) {
		...
		if ( 'yes' != get_option( 'wcvendors_hide_product_general_price' ) && 'yes' != get_option( 'wcvendors_hide_product_general_sale_price' ) ) {
			...
			$required_field    = 'yes' === get_option( 'wcvendors_required_product_general_sale_price' ) ? array(
				'required'                   => '',
	...
	public static function download_limit( $post_id ) {
		...
		if ( 'yes' != get_option( 'wcvendors_hide_product_general_download_files' ) && 'yes' != get_option( 'wcvendors_hide_product_general_download_limit' ) ) {
			...
			$product           = ( is_numeric( $post_id ) ) ? wc_get_product( $post_id ) : null;
			$required_field    = 'yes' === get_option( 'wcvendors_required_product_general_download_limit' ) ? array(
				'required'                   => '',
	...
	public static function download_expiry( $post_id ) {
		...
		if ( 'yes' != get_option( 'wcvendors_hide_product_general_download_files' ) && 'yes' != get_option( 'wcvendors_hide_product_general_download_expiry' ) ) {
			...
			$product           = ( is_numeric( $post_id ) ) ? wc_get_product( $post_id ) : null;
			$required_field    = 'yes' === get_option( 'wcvendors_required_product_general_download_expiry' ) ? array(
				'required'                   => '',
	...
	public static function download_type( $post_id ) {
		...
		if ( 'yes' != get_option( 'wcvendors_hide_product_general_download_files' ) && 'yes' != get_option( 'wcvendors_hide_product_general_download_type' ) ) {
			...
			$custom_attributes = 'yes' === get_option( 'wcvendors_required_product_general_download_type' ) ? array(
				'required'                   => '',
	 ...
	 * @param     int $post_id post_id for this meta if any
	 */
	public static function sku( $post_id, $desc = 'true' ) {
		...
		if ( ! $hide_sku && ! wc_string_to_bool( get_option( 'wcvendors_hide_product_general_sku', 'no' ) ) ) {
			...
			if ( wc_product_sku_enabled() ) {
				...
				$custom_attributes = 'yes' === get_option( 'wcvendors_required_product_general_sku' ) ? array(
					'required'                   => '',
	...
	public static function private_listing( $post_id ) {
		...
		if ( 'yes' != get_option( 'wcvendors_hide_product_general_private_listing' ) ) {
			...
			$custom_attributes = 'yes' === get_option( 'wcvendors_required_product_general_private_listing' ) ? array(
				'required'                   => '',
	...
	public static function button_text( $post_id ) {
		...
		if ( 'yes' != get_option( 'wcvendors_hide_product_general_button_text' ) ) {
			...
			$custom_attributes = 'yes' === get_option( 'wcvendors_required_product_general_button_text' ) ? array(
				'required'                   => '',
	...
	public static function tax( $post_id ) {
		...
		if ( 'yes' != get_option( 'wcvendors_hide_product_general_tax' ) ) {
			...
			if ( wc_tax_enabled() ) {
				...
				$custom_attributes = 'yes' === get_option( 'wcvendors_required_product_general_tax' ) ? array(
					'required'                   => '',
	...
	public static function stock_status( $post_id ) {
		...
		if ( 'yes' != get_option( 'wcvendors_hide_product_inventory_manage_inventory' ) && 'yes' != get_option( 'wcvendors_hide_product_inventory_stock_status' ) ) {
			...
			$custom_attributes = 'yes' != get_option( 'wcvendors_required_product_inventory_stock_status' ) ? array(
				'required'                   => '',
	...
	public static function sold_individually( $post_id ) {
		...
		if ( 'yes' != get_option( 'wcvendors_hide_product_inventory_manage_inventory' ) && 'yes' != get_option( 'wcvendors_hide_product_inventory_sold_individually' ) ) {
			...
			$custom_attributes = 'yes' === get_option( 'wcvendors_required_product_inventory_sold_individually' ) ? array(
				'required'                   => '',
	...
	public static function low_stock_threshold( $post_id ) {
		...
		if ( 'yes' != get_option( 'wcvendors_hide_product_inventory_manage_inventory' ) && 'yes' != get_option( 'wcvendors_hide_product_inventory_low_stock_threshold' ) ) {

			$custom_attributes = 'yes' === get_option( 'wcvendors_required_product_inventory_low_stock_threshold' ) ? array(
				'required'                   => '',
	...
	public static function weight( $post_id ) {
		...
		if ( 'yes' != get_option( 'wcvendors_hide_product_shipping_weight' ) ) {
			...
			if ( wc_product_weight_enabled() ) {
				...
				$custom_attributes = 'yes' === get_option( 'wcvendors_required_product_shipping_weight' ) ? array(
					'required'                   => '',
	...
	public static function dimensions( $post_id ) {
		...
		if ( 'yes' != get_option( 'wcvendors_hide_product_shipping_dimensions' ) ) {
			...
			if ( wc_product_dimensions_enabled() ) {
				...
				$length_custom_attributes = 'yes' === get_option( 'wcvendors_required_product_shipping_dimensions' ) ? array(
					'required'                   => '',
				...
				$width_custom_attributes = 'yes' === get_option( 'wcvendors_required_product_shipping_dimensions' ) ? array(
					'required'                   => '',
				...
				$height_custom_attributes = 'yes' === get_option( 'wcvendors_required_product_shipping_dimensions' ) ? array(
					'required'                   => '',
	...
	public static function up_sells( $post_id ) {
		...
		if ( 'yes' != get_option( 'wcvendors_hide_product_upsells_up_sells' ) ) {
			...
			$required_field = 'yes' === get_option( 'wcvendors_required_product_upsells_up_sells' ) ? array(
				'required'                   => '',
	...
	public static function crosssells( $post_id ) {
		...
		if ( 'yes' != get_option( 'wcvendors_hide_product_upsells_crosssells' ) ) {
			...
			$required_field = 'yes' === get_option( 'wcvendors_required_product_upsells_crosssells' ) ? array(
				'required'                   => '',
				'data-parsley-error-message' => __( 'Please select a product.', 'wcvendors-pro' ),
			) : array();
...
//				),
		...
		if ( wc_string_to_bool( get_option( 'wcvendors_hide_product_seo' ) ) ) {
			unset( $product_meta_tabs['seo'] );
		}

		// Hide Attributes tab if it's set to hidden.
		if ( wc_string_to_bool( get_option( 'wcvendors_hide_product_basic_attributes' ) ) ) {
