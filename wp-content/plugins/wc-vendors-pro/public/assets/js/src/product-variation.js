	...
	*/
	...
	var wcv_product_variations_actions = {
		...
		variable_manage_stock: function() {
			...
			$( this ).closest( '.wcv_variation' ).find( '.hide_if_variation_manage_stock' ).show();

			if ( $( this ).is( ':checked' ) ) {
				$( this ).closest( '.wcv_variation' ).find( '.show_if_variation_manage_stock' ).show();
				$( this ).closest( '.wcv_variation' ).find( '.hide_if_variation_manage_stock' ).hide();
			}
		},
	...
	var wcv_product_variations_ajax = {
		...
		init: function() {
			...
			$( '.wcv-metaboxes-wrapper' ).on( 'change', 'select.variation_grouped_actions', function() {
		...
		*/
		...
		do_variation_action: function( current_select_id ) {
			...
			if ( do_variation_action == '-1' ) return;
			...
			switch ( do_variation_action ) {
				...
				default :
					...
					data = $( 'select#' + current_select_id ).triggerHandler( do_variation_action + '_ajax_data', data );
					break;
			}
