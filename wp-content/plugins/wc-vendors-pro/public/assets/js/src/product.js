...
jQuery( function( $ ){
	...
	$( 'select#product-type' ).change( function () {
		...
		// Re init select2
		$( '.wcv-tabs' ).find( 'select.select2' ).select2();
	...
	function show_and_hide_panels() {
		...
		if ( product_type == "grouped" ) {
			 ...
			 $( '.linked_product' ).parent().addClass('active' );
			 $( '#linked_product' ).removeClass('hide-all').addClass('active');
	...
	$( document ).on( 'click', function(e){
		...
		var max_selected_notice = $( '#product_images_container' ).data( 'gallery_max_selected_notice' );

		if ( selected_count > gallery_max ) {
			$( '.max-gallery-selected-warning' ).remove();
			$( '.media-toolbar-secondary').prepend( '<div class="woocommerce-info max-gallery-selected-warning">' + max_selected_notice + '</div>' );
	...
	function product_gallery_uploader() {
	    ...
	    media_uploader.on( 'select' , function( ) {
			...
			if ( count_selected_attachments > 3 ) {
				var message = '<div class="max-gallery-images-exceeded-warning">' + max_selected_notice + '</div>';
				$( '#product_images_container' ).append( message );
			}
		});

		media_uploader.on( 'close', function(e) {
	...
	$('.category-select2').select2(
		{
			...
			language: {
			    maximumSelected: function (args) { return (args.maximum != 1) ? wcv_frontend_product.select2_maximumSelected_plural : wcv_frontend_product.select2_maximumSelected_single ; },
			    loadingMore: function () { return wcv_frontend_product.select2_loadingMore; },
			    errorLoading: function () { return wcv_frontend_product.select2_errorLoading; },
			    noResults: function () { return wcv_frontend_product.select2_noResults; },
			    searching: function () { return wcv_frontend_product.select2_searching; },
			    removeAllItems: function () { return wcv_frontend_product.select2_removeAllItems; }
			}
		}
	);
			...
			if ( attribute ) {
				...
				$.post( wcv_frontend_product.ajax_url, data, function( response ) {
					...
					if ( response.error ) {
						...
						$( 'select.attribute_values' ).filter( ':not(.select12-hidden-accessible)' ).select2();
			...
			if ( wcv_frontend_product.require_attributes == 1 ){
				...
				if ( attribute_container_count > 0 ) {
					...
					$( '.wcv_attribute_data').each( function( index ) {
						...
						if ( ! $( this ).find( '.attribute_values').val() ){

							$( '.attributes-validation-error').html( '<span class="wcv_required_form_msg">' + wcv_frontend_product.require_attributes_msg + '</span>');
					...
					$( '.attributes-validation-error').html( '<span class="wcv_required_form_msg">' + wcv_frontend_product.require_attributes_msg + '</span><br />');
					$( 'a.tabs-tab.attributes').css( 'background-color', 'rgba(150,8,12,.55)' );
					$( 'a.tabs-tab.attributes').css( 'color', 'rgba(255,255,255,.55)' );
					e.preventDefault();
				}
