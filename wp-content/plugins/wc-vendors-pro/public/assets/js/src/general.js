jQuery( function( $ ){
	...
	function file_uploader(id) {
		...
		media_uploader.on('select', function () {
			...
			if (file_type === 'image') {
				var attachment_image_url = json.sizes.thumbnail ? json.sizes.thumbnail.url : json.url;
			...
			if (file_type === 'video' || file_type === 'audio' || file_type === 'document') {
				var data = {
			...
			$('.wcv-file-uploader' + id)
				.append('<div class="control-group"><div class="control"><input type="text" value="' + json.url + '" readonly/></div></div>');
	...
	window.Parsley.on('field:error', function() {
  		...
  		$( '.wcv-tabs' ).find( '.' + $parent_tab_id ).addClass( 'tab-has-error' );
	});

	window.Parsley.on('field:success', function () {
		var $parent_tab = this.$element.closest('.tabs-content');

		if ($parent_tab.find('.parsley-error').length > 0) return;

		var $parent_tab_id = $parent_tab.attr('id');
		$('.wcv-tabs').find('.' + $parent_tab_id).removeClass('tab-has-error');
	});
