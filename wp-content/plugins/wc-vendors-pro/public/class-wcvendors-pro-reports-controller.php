...
class WCVendors_Pro_Reports_Controller {
	...
	public function recent_orders_table() {
		...
		$columns = array(
			...
			'product'      => __( 'Item/s', 'wcvendors-pro' ),
			'net'      => __( 'Net', 'wcvendors-pro' ),
			'price'      => __( 'Price', 'wcvendors-pro' ),
			'totals'       => __( 'Total', 'wcvendors-pro' ),
		...
		if ( ! empty( $recent_orders ) ) {
			...
			foreach ( $recent_orders as $order ) {
                ...
                $new_row->net       = wc_price($order->order->total);
                $new_row->price       = wc_price($subtotal);
                $new_row->totals       = $totals_html;
				$new_row->status       = $order->order->status;
				$new_row->date       = date_i18n( 'd/m/y', strtotime( $order->recorded_time ) );

				$new_row->product_id       = reset(array_keys($order->order_items));
	...
	public function recent_products_table() {
		...
		// display the table
		$recent_product_table->display(false);
