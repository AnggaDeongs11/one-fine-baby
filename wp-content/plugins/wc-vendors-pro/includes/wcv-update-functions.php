...
function wcv_fix_product_form() {
	$product_form = get_option( 'wcvendors_product_form_template' );
...
function add_defaults_google_maps_options() {
	if ( get_option( 'wcvendors_pro_google_maps_zoom_level' ) === false ) {
		update_option( 'wcvendors_pro_google_maps_zoom_level', 18 );
	}

	if ( get_option( 'wcvendors_pro_location_picker_default_visibility' ) === false ) {
...
function wcv_add_variation_dropdown_type_option() {
	if ( ! get_option( 'wcvendors_hide_product_variations_download_expiry' ) ) {
...
function add_vacation_disable_cart_defaults() {
	...
	$hide_totals_fields  = get_option( 'wcvendors_hide_settings_store_product_total_sales' );
	$store_sales_label   = get_option( 'wcvendors_store_total_sales_label' );
	$product_sales_label = get_option( 'wcvendors_product_total_sales_label' );
	$override_vendor     = get_option( 'wcvendors_override_vendor_total_sales_label' );
...
function add_store_total_sales_options() {
	...
	$show_store_totals = get_option( 'wcvendors_show_store_total_sales' );
	if ( false === $show_store_totals ) {
		add_option( 'wcvendors_show_store_total_sales', 'yes' );
	}

	$show_product_totals = get_option( 'wcvendors_show_product_total_sales' );
...
function add_hide_vendor_store_notice_option() {
	if ( false === get_option( 'wcvendors_hide_settings_vendor_store_notice' ) ) {
		add_option( 'wcvendors_hide_settings_vendor_store_notice', 'no' );
	}

	if ( false === get_option( 'wcvendors_hide_settings_store_store_enable_notice' ) ) {
...
function add_options_for_vendor_shipping_type() {
	if ( false === get_option('wcvendors_hide_settings_store_shipping_type') ) {
		add_option( 'wcvendors_hide_settings_store_shipping_type', 'yes');
		add_option( 'wcvendors_required_settings_store_shipping_type', 'no');
	}

	if ( false === get_option('wcvendors_hide_signup_store_shipping_type') ) {
...
function add_sync_reviews_option() {
	if ( false == get_option( 'wcvendors_feedback_sync_reviews' ) ) {
...
function add_upload_limits() {
	if ( false === get_option( 'wcvendors_global_disk_usage_limit' ) ) {
...
function add_product_seo_options() {
	if (false == get_option('wcvendors_hide_product_seo_title')){
...
function add_remove_product_tab_option() {
	if ( false === get_option( 'wcvendors_feedback_remove_product_vendor_ratings_tab' ) ) {
