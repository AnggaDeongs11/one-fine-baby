...
class WCVendors_Pro_Upload_Limits
{
	...
	public function get_media( $user_id = 0 ) {
		...
		if ( $_user_media = get_transient( 'wcv_user_media_' . $user_id ) ) {
			return $_user_media;
		}
		...
		$the_query = new WP_Query(
			apply_filters( 'wcv_get_user_media_query_args', array(
		...
		if ( $the_query->have_posts() ) {
			...
			while ( $the_query->have_posts() ) :
				$the_query->the_post();
				$file_path = str_replace( site_url( ), ABSPATH, get_the_guid() );
    ...
    public function include_vendor_thumbnails( $user_id = 0) {
        ...
        return wc_string_to_bool( apply_filters(
	 ...
	 * @return int|string
	 */
	public function format_disk_usage_size( $size ) {
		if ( $size == 0 ) {
			return 0;
		}

		return $this->format_disk_size( $size );
	}
	 ...
	 * @version 1.6.0
	 */
	public function format_disk_size($size)
	{
		if ( $size == 0 ) {
